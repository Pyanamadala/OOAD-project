	...
	void deactivateRCM(String status){
		try {
			...
			if(status.equals("AMOUNT")){
				sql="update rcmdetails set isAmtEmpty=1 where rcmid="+this.rcmID;
				stmt.executeUpdate(sql);
			}
			...
			if(status.equals("COUPON")){
				sql="update rcmdetails set isCouponEmpty=1 where rcmid="+this.rcmID;
				stmt.executeUpdate(sql);
			}
			if(status.equals("DEACTIVATE")){
				sql="update rcmdetails set activation=0,isCouponEmpty=1, isAmtEmpty=1 where rcmid="+this.rcmID;
				stmt.executeUpdate(sql);
			}
